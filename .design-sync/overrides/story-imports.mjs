  ...
  const policyRedirect = {
    ...
    setup(b) {
      b.onResolve({ filter: /.*/ }, async (a) => {
        ...
        // PERF FORK: DS import policy (shim / source-barrel redirect) only ever
        // applies to the story file's OWN imports and the package's own source —
        // never to imports made from INSIDE a third-party package. Skipping
        // node_modules importers lets esbuild resolve their internals natively
        // instead of paying an async b.resolve round-trip per module. Without
        // this, lucide-react's ~1600-icon barrel made each preview esbuild
        // bundle take ~90s (plain esbuild: 0.4s); 44 previews => ~70min builds.
